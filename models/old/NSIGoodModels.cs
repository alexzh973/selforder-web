...
namespace wstcp.Models
{
    ...
    public abstract class GoodInfo
    {
        ...
        public static GoodOwnInfo GetInfo(int id,int owner_id)
        {
            ...
            string str = (owner_id>0)? "select good.id, good.ens, good.img, good.Article, good.Name, good.xTN, good.xTK, good.Brend, g.GoodCode, good.ed, g.Qty, g.zn, g.zn_z,g.zt, g.pr_spr,g.pr_b, g.lcd from OWNG as g inner join GOOD on good.id=g.GoodID where g.GoodID=" + id+" and g.OwnerId="+owner_id +" ":
                "select good.id, good.ens, good.img,good.Article, good.Name, good.xTN, good.xTK, good.Brend, good.ed, (SELECT TOP (1) GoodCode AS Expr1 FROM OWNG  WHERE (GoodId = GOOD.ID) AND (ISNULL(GoodCode, '') <> '')  ORDER BY lcd DESC) AS  GoodCode, isnull(sum(g.Qty),0) as Qty, '' as zn, '' as zn_z,'' as zt, isnull(max(g.pr_spr),0) as pr_spr,isnull(max(g.pr_b),0) as pr_b, max(g.lcd) as lcd from GOOD inner join OWNG as g on good.id=g.GoodID where good.ID=" + id + " group by good.ENS, good.ID,good.Article, good.Name,good.xTN,good.xTK, good.Brend, good.ed";
            SqlConnection cn = new SqlConnection(db.DefaultCnString);
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = str;
            cn.Open();
        ...
        {
            GoodOwnInfo go;// = new GoodOwnInfo();
            string str = (owner_id>0)? "select good.Article, good.ens,good.img,good.Name,good.xTN, good.xTK, good.ed, good.Brend, g.GoodCode, g.Qty, g.zn, g.zn_z, g.zt, g.pr_spr, g.lcd from OWNG as g inner join GOOD on good.id=g.GoodID where g.GoodCode=" + code+" and g.OwnerId="+owner_id +" ":
                "select good.Article, good.ens, good.img,good.Name,good.xTN, good.xTK, good.ed, good.Brend, '' as GoodCode, isnull(sum(g.Qty),0) as Qty, '' as zn, '' as zn_z, '' as zt, isnull(max(g.pr_spr),0) as pr_spr, max(g.lcd) as lcd from GOOD inner join OWNG as g on good.id=g.GoodID where g.GoodCode=" + code + " group by good.ID,good.Article, good.Name,good.xTN, good.xTK, good.ed, good.Brend";
            SqlConnection cn = new SqlConnection(db.DefaultCnString);
            SqlCommand cmd = cn.CreateCommand();
            cmd.CommandText = str;
            cn.Open();
