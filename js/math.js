...
function calcByKrat(inp, krat) {
    var q = myroundEx((inp / krat), 0);
    return (q < 1) ? krat : q * krat;
}


function cToDecimal(val) {
    return myParseFloat(val);
}
function cToDecimal(val,qtyD) {
    val = myParseFloat(val);
    return myroundEx(val,qtyD);
}
...
function checkNumeric(nameField, defValue) {
    ...
    valueField = valueField.replace(" ", "").replace(WrongDigitSymbol, DigitSymbol);
    var v = myroundEx(valueField, 2);
    $("#" + nameField).val((''+v == 'NaN') ? defValue : v);
}


function myParseFloat(val) {
	return parseFloat(myroundEx(String(val).replace(' ','').replace(WrongDigitSymbol, DigitSymbol),7));
}
